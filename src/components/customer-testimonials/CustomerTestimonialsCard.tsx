...
const CustomerTestimonialsCard = (model: CustomerTestimonialsCardModel) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "customer-testimonials-card-play absolute left-1/2 top-1/2 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-brand-gold shadow-md transition-transform duration-200 group-hover:scale-110 group-hover:shadow-lg",
        )}
        aria-hidden="true"
      >
        <Play className="h-6 w-6 fill-brand-white text-brand-white" strokeWidth={0} />
      </span>
    </button>
  );
};
